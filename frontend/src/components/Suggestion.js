...
const  Suggestion = () =>{
    ...
    const currentOption = techStacks?techStacks.find((stck)=> stck.id == type):{}
    ...
    const options =techStacks?techStacks.map((stack)=>{return {"option":stack.participant + " - " + stack.model, "id":stack.id}}):[]
    //console.log(options);
    ...
    const submitHandler = (e) => {
      ...
      dispatch(updateSuggestionAction(currentOption.id, currentOption.participant, currentOption.model, lidar, camera, radar, level, compute, providingService, source));
    ...
    useEffect(() => {
        ...
        setProvidingService(currentOption?(currentOption.providingService.replace("|","\n-")):"")
        ...
        return (
            <div>
            <Button onClick= {()=> setShowSuggestion(!showSuggestion)}>Make a Suggestion</Button>
            { (!showSuggestion && suggestion)?<h1>Thank you! After a review we will add your contribution.</h1>:<h1></h1>}
            ...
            >
              ...
              {options.map((opt)=> <option key={opt._id} value={opt.id}>{opt.option}</option>)}
            </Form.Control>
                ...
                {suggestionError && <ErrorMessage>Please select an entry</ErrorMessage>}
